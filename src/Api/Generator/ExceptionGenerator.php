...
class ExceptionGenerator extends \Jane\Component\OpenApiCommon\Generator\ExceptionGenerator
{
    ...
    public function generate(string $functionName, int $status, Context $context, ?ClassGuess $classGuess, bool $isArray, ?string $classFqdn, ?string $description): ?string
    {
        ...
        $highLevelExceptionName = $this->createHighLevelException($context, $status, $classGuess, $isArray, $classFqdn);
...
EOD
    ...
    private function createHighLevelException(Context $context, int $code, ?ClassGuess $classGuess, bool $isArray, ?string $classFqdn): string
    {
        $schema = $context->getCurrentSchema();
