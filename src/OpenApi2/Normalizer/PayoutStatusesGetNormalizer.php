...
class PayoutStatusesGetNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return 'CatchE\\OpenApi2\\Model\\PayoutStatusesGet' === $type;
    }

    public function supportsNormalization($data, $format = null)
    {
        return is_object($data) && 'CatchE\\OpenApi2\\Model\\PayoutStatusesGet' === get_class($data);
    }
    ...
    public function denormalize($data, $class, $format = null, array $context = [])
    {
        ...
        $object = new \CatchE\OpenApi2\Model\PayoutStatusesGet();
        if (null === $data || false === \is_array($data)) {
            return $object;
        }
        if (\array_key_exists('_links', $data) && null !== $data['_links']) {
            $object->setLinks($this->denormalizer->denormalize($data['_links'], 'CatchE\\OpenApi2\\Model\\PayoutStatusesGetLinks', 'json', $context));
        } elseif (\array_key_exists('_links', $data) && null === $data['_links']) {
            $object->setLinks(null);
        }
        if (\array_key_exists('_embedded', $data) && null !== $data['_embedded']) {
            $object->setEmbedded($this->denormalizer->denormalize($data['_embedded'], 'CatchE\\OpenApi2\\Model\\PayoutStatusesGetEmbedded', 'json', $context));
